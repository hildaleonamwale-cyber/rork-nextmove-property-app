-- ============================================
-- COMPLETE SUPABASE SCHEMA - FIXED VERSION
-- All tables, policies, triggers, and storage configuration
-- Fixes: Added conversations table, fixed column names
...
-- ============================================
-- CONVERSATIONS TABLE (NEW - FIXES ERROR)
-- ============================================
...
-- ============================================
-- MESSAGES TABLE (UPDATED TO USE CONVERSATIONS)
-- ============================================
...
-- Function to notify on property changes
...
DROP TRIGGER IF EXISTS on_property_change ON properties;
CREATE TRIGGER on_property_change
  AFTER INSERT OR UPDATE OR DELETE ON properties
  FOR EACH ROW
  EXECUTE FUNCTION notify_property_change();

-- Function to notify on agent profile changes
...
DROP TRIGGER IF EXISTS on_agent_change ON agents;
...
BEGIN
  ...
  RAISE NOTICE '   - conversations (NEW), bookings, messages, notifications, wishlists';
  ...
  RAISE NOTICE '📡 Realtime triggers for messages, properties, and agents';
  RAISE NOTICE '📈 Analytics views created';
  RAISE NOTICE '';
  RAISE NOTICE '✨ Database is ready to use!';
  RAISE NOTICE '';
  RAISE NOTICE '🔧 FIXES APPLIED:';
  RAISE NOTICE '   - Added conversations table with proper relationships';
  RAISE NOTICE '   - Fixed properties table (uses bedrooms/bathrooms, not beds/baths)';
  RAISE NOTICE '   - Added realtime publication for key tables';
  RAISE NOTICE '   - Hardened functions with SET search_path';
END $$;
