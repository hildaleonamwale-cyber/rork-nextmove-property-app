...
export default function RoomCard({ room, onPress, variant = 'default' }: RoomCardProps) {
  ...
  const isNew = room.createdAt ? (Date.now() - new Date(room.createdAt).getTime() < 7 * 24 * 60 * 60 * 1000) : false;
        ...
        <View style={styles.badgeRow}>
          ...
          {room.lister && (
            <View style={styles.listerBadgeContainer}>
              {room.lister.type === 'company' && room.lister.companyLogo ? (
                <Image source={{ uri: room.lister.companyLogo }} style={styles.listerBadge} />
              ) : (
      ...
      <View style={styles.content}>
        ...
        <View style={styles.priceRow}>
          <Text style={styles.price}>
            ${room.price.toLocaleString()}{room.priceType === 'monthly' ? '/month' : ''}
          </Text>
          {isNew && (
