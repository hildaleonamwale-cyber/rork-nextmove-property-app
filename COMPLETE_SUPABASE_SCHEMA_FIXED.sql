-- ============================================
-- COMPLETE SUPABASE SCHEMA - INFINITE RECURSION FIX
-- Execute this to reset and recreate all tables
-- ============================================
...
DROP POLICY IF EXISTS "admins_manage_properties" ON properties;
DROP POLICY IF EXISTS "bookings_select_own" ON bookings;
DROP POLICY IF EXISTS "bookings_select_property_owner" ON bookings;
DROP POLICY IF EXISTS "bookings_insert_own" ON bookings;
DROP POLICY IF EXISTS "bookings_update_own" ON bookings;
DROP POLICY IF EXISTS "bookings_update_property_owner" ON bookings;
...
DROP POLICY IF EXISTS "banners_select_enabled" ON banners;
DROP POLICY IF EXISTS "banners_select_all_admin" ON banners;
DROP POLICY IF EXISTS "admins_manage_banners" ON banners;
DROP POLICY IF EXISTS "sections_select_enabled" ON homepage_sections;
DROP POLICY IF EXISTS "sections_select_all_admin" ON homepage_sections;
DROP POLICY IF EXISTS "admins_manage_sections" ON homepage_sections;
...
-- ============================================
-- Allow users to read all user data (for displaying profiles)
...
DO $$
BEGIN
  RAISE NOTICE '✅ Schema created successfully with infinite recursion fix!';
  ...
  RAISE NOTICE '🔒 RLS policies simplified to prevent circular dependencies';
  RAISE NOTICE '   - Users: Public read + own update + service role';
  RAISE NOTICE '   - Agents: Public read + own CRUD + service role';
  RAISE NOTICE '   - Properties: Public read + owner CRUD + service role';
  RAISE NOTICE '   - Bookings: Own + property owner access + service role';
  RAISE NOTICE '   - Admin operations now handled via service role';
  RAISE NOTICE '🔄 Auto-update triggers configured';
  RAISE NOTICE '📈 Analytics views created';
  RAISE NOTICE '🔐 Auth triggers configured for automatic user creation';
END $$;
